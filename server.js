...
if (currentDate !== lastDate) {
    ...
    if (currentDay === 1 && currentDate !== lastWeekMonday) {
        console.log("WEEK CHANGED: Processing weekly data for all meters.");
        await calculateAndInsertWeeklyDataForAllMeters();
        lastWeekMonday = currentDate;
      }
    ...
    if (currentDayOfMonth === 1 && currentMonth !== lastMonth) {
        console.log("MONTH CHANGED: Processing monthly data for all meters.");
        await calculateAndInsertMonthlyDataForAllMeters();
        lastMonth = currentMonth;
      }
...
async function calculateAndInsertWeeklyDataForAllMeters() {
  try {
    const pool = await sql.connect(sqlConfig);
    const energyConsumptionTable = `[EMS_thermax].[dbo].[energy_consumption]`;
    const energyConsumptionWeeklyTable = `[EMS_thermax].[dbo].[energy_consumption_weekly]`;
    ...
    for (const meterId of meter_id_list) {
      try {
        ...
        const weeklyQuery = `
          ...
          WHERE 
            meter_id = ${meterId}
            AND CONVERT(VARCHAR(10), TRY_CAST(date_time AS DATETIME), 120) BETWEEN '${startDate}' AND '${endDate}'
          GROUP BY meter_id;
        `;
    ...
    console.log("Weekly data processed successfully for all meters.");
  } catch (err) {
    console.error("Error processing weekly data for all meters:", err);
  }
}
...
async function calculateAndInsertMonthlyDataForAllMeters() {
  try {
    const pool = await sql.connect(sqlConfig);
    const energyConsumptionWeeklyTable = `[EMS_thermax].[dbo].[energy_consumption]`;
    const energyConsumptionMonthlyTable = `[EMS_thermax].[dbo].[energy_consumption_monthly]`;
    ...
    for (const meterId of meter_id_list) {
      try {
        ...
        // Query to calculate monthly averages and sums
        const monthlyQuery = `
          SELECT 
            meter_id,
            MAX(load_name) AS load_name,
        ...
        for (const monthlyData of monthlyResult.recordset) {
          const {
            meter_id, load_name, avgKWh, sumKwhC,
            avgVry, avgVyb, avgVbr,
            avgVLN1, avgVLN2, avgVLN3,
            avgIr, avgIy, avgIb,
            avgPF, avgF, avgKW, avgKVAh, avgMD,
    ...
    console.log("Monthly data processed successfully for all meters.");
  } catch (err) {
    console.error("Error processing monthly data for all meters:", err);
  }
}
